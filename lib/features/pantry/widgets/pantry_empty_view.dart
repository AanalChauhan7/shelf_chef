...
class _PantryEmptyViewState extends State<PantryEmptyView> {
  ...
  List<PantryItem> _apiItems = [];
  bool _isLoading = false;

  final List<PantryItem> _fallbackItems = [
    PantryItem(
      id: '1',
      name: 'Fresh Tomatoes',
      category: 'Vegetables',
  ...
  final List<Map<String, dynamic>> _categories = const [
    ...
    {'name': 'Fruit', 'icon': Icons.apple_rounded},
  ...
  Future<void> _loadItemsFromApi() async {
    ...
    final fetched = await PantryApiService.fetchPantryItems();
    if (mounted) {
      setState(() {
        _apiItems = fetched;
        _isLoading = false;
  ...
  List<PantryItem> get _displayItems =>
      _apiItems.isNotEmpty ? _apiItems : _fallbackItems;
  ...
  Widget build(BuildContext context) {
    ...
    final filteredItems = _displayItems.where((item) {
    ...
    return SafeArea(
      child: RefreshIndicator(
        ...
        child: ListView(
          ...
          children: [
            PantryHeaderBanner(
              totalItemsCount: _displayItems.length,
              expiringSoonCount:
                  _displayItems.where((i) => i.isExpiringSoon).length,
            ),
